...
:::tip
For the potential enhancements you can on your own in your applications as suggested above is you wish or if you would like to see this inside **Arkos** just drop an issue with the an PR if you want to implement the code inside **Arkos**. [Github Issues](https://github.com/superm7/arkos/issues)
:::

## Using Auth Config To Customize Endpoint Behavior

By default **Arkos** will require authentication in all endpoints and will only allow the user with the field `isSuperUser` set to `true` be able to operate on the API, and in most of the scenarios will want to customize your endpoint access control using the role.

Hence **Arkos** provides a simple and intuitive way to customize access control through Static Role-Based Acess Control using `auth-configs.ts` files.
